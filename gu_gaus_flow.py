...
# model parameters
parser.add_argument('--input_size', type=int, default=1, help='Input size in a model.')
parser.add_argument('--layer', type=int, default=5, help="Total number of Gaussianization layers")
parser.add_argument('--kde_num', type=int, default=1, help='Stacking multiple KDE layers before each rotation layer')
parser.add_argument('--total_datapoints', type=int, default=100, help="Total number of data points for each KDE layer")
parser.add_argument('--usehouseholder', action='store_true', help='Train rotation matrix using householder reflection')
parser.add_argument('--multidim_kernel', action='store_true', help='Use multiple dimension bandwidth kernel')
...
parser.add_argument('--process_size', type=int, default=100, help="Process size")
parser.add_argument('--adjust_step', type=int, default=10000, help="Decrease learning rate after a couple steps")

parser.add_argument('--log_interval', type=int, default=1000, help='How often to show loss statistics and save models/samples.')

parser.add_argument('--cuda', type=int, default=0, help='Number of CUDA to use if available.')
    ...
    for i in range(1, args.niters + 1):
        ...
        if i % args.log_interval == 0:
            ...
            with torch.no_grad():
                ...
                fake = model.sampling(DATA, prior, process_size=args.eval_size, sample_num=args.eval_size)
...
if __name__ == '__main__':
    ...
    # model
    model = Net(args.total_datapoints, args.layer, args.input_size, args.kde_num, multidim_kernel=args.multidim_kernel,
